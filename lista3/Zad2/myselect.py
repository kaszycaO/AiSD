...
# same algorithm but without log
def my_select_stat(my_list, element):
    copy = my_list.copy()
    result = do_select_stat(my_list, element)
    index = my_list.index(result)
    my_list[index] = [my_list[index]]


def do_select_stat(my_list, element):
    global compares

    groups = divide(my_list)
    medians = list([get_median_stat(group) for group in groups])

    if len(medians) <= 5:
        pivot =  get_median_stat(medians)
    else:
        pivot =  do_select_stat(medians, len(medians)//2)

    pivot_index = partition_stat(my_list, 0, len(my_list)-1, pivot)

    # +1 wynika z roznicy w indeksowaniu
    if element < pivot_index+1:
        return do_select_stat(my_list[:pivot_index], element)
    elif element > pivot_index+1:
        return do_select_stat(my_list[pivot_index+1:], element-pivot_index-1)
    else:
        return pivot


def get_median_stat(group):
    insertion_sort_stat(group)
    index = len(group)//2
    if len(group) % 2 == 0:
        return min(group[index], group[index - 1])
    else:
...
def partition_stat(my_list, low, high, pivot):
    global compares
    if low == high:
        return 0

    my_index = my_list.index(pivot)
    swap_elements_stat(my_list, low, my_index)
    i = low
    j = high + 1
    counter = 0
    while True:
        ...
        if i < j:
            ...
            swap_elements_stat(my_list, i - 1, low)
            return j


def insertion_sort_stat(my_list):
    global swaps
    global compares
